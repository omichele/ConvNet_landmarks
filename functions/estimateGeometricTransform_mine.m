function [tform, inlierPoints1, inlierPoints2, inlier_ind, status] ...
    = estimateGeometricTransform_mine(matchedPoints1, matchedPoints2, ...
    transformType, varargin)
%estimateGeometricTransform Estimate geometric transformation from matching
%point pairs modified to return inlier_ind
...
% Compute the geometric transformation !!!!!!!!!!!  THE CORE
...
if inlierProbability < eps(class(inlierNum))  % that is if the prob is really really small
    N = intmax('int32');
else % otherwise for most of the cases
    conf = cast(0.01, 'like', inlierNum) * confidence; % because confidence is in percentual
...
% core of the msac - computation of the cost function
delta = pt - points2;
dis   = hypot(delta(:,1),delta(:,2)); % square root of sum of squares = e
...
while idxTrial <= numTrials
    ...
    if accDis < bestDis
        ...
        % this is the function that evaluates the quality / likelihood
        % of the inlier set
