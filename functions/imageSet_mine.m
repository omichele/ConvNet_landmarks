...
%                     narginchk(2,2); % cell input can't be followed by anything else
        ...
        function this = parseDirInput(this, varargin)
            ...
            dirName = varargin{1};
            dirName = imageSet_mine.expandDirToAbsPath(dirName);
            ...
            if isRecursive
                ...
                allDirs = [{dirName}, imageSet_mine.getSubDirNames(dirName)];
                
                n = numel(allDirs);
                
                this(n) = imageSet_mine.makeDefaultObject(this); % pre-allocate object array
                for i=1:n
                ...
                this.ImageLocation = imageSet_mine.parseFolder(dirName);
...
%             end
            ...
            dirName = imageSet_mine.expandDirToAbsPath(in);
            
            this.ImageLocation = imageSet_mine.parseFolder(dirName{1});
            
            for i = 2:numel(dirName)
                this.ImageLocation = [this.ImageLocation, imageSet_mine.parseFolder(dirName{i})];
        ...
        function subDirNames = getSubDirNames(rootDir)
            ...
            if ~isempty(subDirNames)
                for i=1:numel(subDirNames)
                    tempDirNames = imageSet_mine.getSubDirNames(subDirNames{i});
    ...
    methods (Access = public)
        ...
        %------------------------------------------------------------------
        function this = imageSet_mine(varargin)
    ...
    methods(Access = public, Sealed)
        ...
        function out = select(this, index)
            ...
            out(n) = imageSet_mine.makeDefaultObject(this);
        ...
        function varargout = partition(this, groupSizes, varargin)
            ...
            for setIdx = 1:numSets;
                ...
                for splitIdx = 1:numSplits
                    ...
                    % make a split
                    if isempty(range)
                        varargout{splitIdx} = [varargout{splitIdx}, imageSet_mine.makeDefaultObject(this)];
